"""
Módulo cliente para o jogo 'Flappy Priolo - Azores Edition'.
Gere a interface gráfica em Pygame, a comunicação em rede com o servidor
e a renderização do estado do jogo (multiplayer).
"""
...
def get_asset_path(filename):
    """
    Obtém o caminho absoluto para um ficheiro de recurso (asset).

    Args:
        filename (str): O nome do ficheiro a procurar.

    Returns:
        str: O caminho completo para o ficheiro dentro da pasta 'assets'.
    """
...
def load_gif_frame(filename, size):
    """
    Carrega as frames de um ficheiro GIF animado e converte-as para superfícies do Pygame.

    Args:
        filename (str): O nome do ficheiro GIF.
        size (tuple): O tamanho (largura, altura) pretendido para o redimensionamento.

    Returns:
        list: Uma lista de objetos pygame.Surface correspondentes a cada frame do GIF.
    """
...
def load_and_clean_asset(path, size):
    """
    Carrega uma imagem, remove o espaço transparente em excesso e redimensiona-a.

    Args:
        path (str): O caminho para o ficheiro de imagem.
        size (tuple): O tamanho final pretendido (largura, altura).

    Returns:
        pygame.Surface: A imagem limpa e redimensionada. Devolve um quadrado vermelho
...
def draw_text_shadow(win, text, font, color, x, y):
    """
    Desenha texto com um efeito de sombra no ecrã.

    Args:
        win (pygame.Surface): A superfície (janela) onde o texto será desenhado.
        text (str): O texto a apresentar.
        font (pygame.font.Font): A fonte a utilizar.
        color (tuple): A cor principal do texto (RGB).
...
def draw_styled_button(win, rect, text, font, base_color, hover_color, text_color):
    """
    Desenha um botão estilizado com efeitos de interação (hover) consoante a posição do rato.

    Args:
        win (pygame.Surface): A janela onde o botão será desenhado.
        rect (pygame.Rect): A área e posição do botão.
        text (str): O texto a exibir dentro do botão.
        font (pygame.font.Font): A fonte do texto.
        base_color (tuple): A cor base do botão (RGB).
        hover_color (tuple): A cor do botão quando o rato passa por cima (RGB).
        text_color (tuple): A cor do texto (RGB).
    """
...
class InterfaceGrafica:
    """
    Classe responsável por gerir a interface gráfica e o ciclo principal do jogo cliente.
    """

    def __init__(self):
        """
        Inicializa o Pygame, configura a janela, carrega os recursos gráficos,
        as fontes e define o estado inicial das variáveis.
        """
        pygame.init()
    ...
    def ask_for_name(self, bg_idx):
        """
        Apresenta um ecrã para o jogador inserir o seu nome de utilizador.

        Args:
            bg_idx (int): O índice atual do frame de fundo animado.

        Returns:
            str: O nome limpo introduzido pelo jogador.
        """
        user_text = ''
    ...
    def draw_game(self, estado, meu_nome, bg_idx):
        """
        Renderiza o estado atual do jogo no ecrã.

        Desenha o fundo, os vulcões, os jogadores (priolos) com animações
        fluidas, a tabela de classificação (leaderboard) e as sobreposições de
        pausa ou depuração (debug), consoante o estado atual da rede.

        Args:
            estado (dict): Os dados de estado recebidos do servidor.
            meu_nome (str): O nome do jogador local.
            bg_idx (int): O índice da frame do fundo a desenhar.
        """
        self.win.blit(self.BACKGROUND_FRAMES[bg_idx], (0, 0))
        ...
        if self.mostrar_debug and 'parametros' in estado:
            ...
            linhas_debug = [
                ...
                f"Distância Vulcões: {p['distancia']}",
    ...
    def execute(self):
        """
        Gere o ciclo de execução principal da aplicação.

        Apresenta o menu inicial, solicita o nome do jogador, tenta estabelecer
        a ligação com o servidor e arranca o ciclo do jogo multiplayer, processando
        os eventos de teclado/rato e atualizando o ecrã.
        """
        while True:
